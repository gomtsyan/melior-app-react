...
# Capture all environment variables starting with MELIOR_ and make JSON string from them
ENV_JSON="$(jq --compact-output --null-input 'env | with_entries(select(.key | startswith("MELIOR_")))')"
